...
function clearContainers() {
  successln "Removing remaining containers"
...
function removeUnwantedImages() {
  successln "Removing generated chaincode docker images"
...
function checkPrereqs() {
  successln "-------- Checando pre requisitos-------------------"
...
# Cria o material criptográfico da organização usando cryptogen ou CA
function createOrgs() {
  if [ -d "organizations/peerOrganizations" ]; then
    infoln " se existe organizations n\ ele deleta"
    rm -Rf organizations/peerOrganizations && rm -Rf organizations/ordererOrganizations
  fi

  # Cria material criptográfico usando cryptogen
  if [ "$CRYPTO" == "cryptogen" ]; then
    infoln "usando cryptogen"

    which cryptogen
    if [ "$?" -ne 0 ]; then
      fatalln "Ferramenta cryptogen não encontrada. Saindo"
    fi
    infoln "============================================="
    infoln "    Temos que crypto === crypptogen"
    infoln "============================================="
    infoln "Gerando certificados usando a ferramenta cryptogen"

    infoln "Criando Identidades Org1"
    infoln "generate --config=./organizations/cryptogen/crypto-config-org1.yaml --output="organizations""
    ...
    { set +x; } 2>/dev/null
    if [ $res -ne 0 ]; then
      fatalln "Falha ao gerar os certificados..."
    fi

    infoln "Criando Identidades Org2"

    set -x
    infoln "generate --config=./organizations/cryptogen/crypto-config-org2.yaml --output="organizations""
    ...
    infoln "Criando Identidades da Ordem"
    infoln "generate --config=./organizations/cryptogen/crypto-config-orderer.yaml --output="organizations""
  ...
  # Cria material criptográfico usando cfssl
  if [ "$CRYPTO" == "cfssl" ]; then
    infoln "============================================="
    infoln "Agora temos que crypto === cfssl"
    infoln "============================================="
    infoln " executando o script organizations/cfssl/registerEnroll.sh"
  ...
  # Cria material criptográfico usando Fabric CA
  if [ "$CRYPTO" == "Certificate Authorities" ]; then
    infoln "============================================="
    fatalln "       Agora temos que crypto === CA"
    infoln "============================================="
    infoln "Gerando certificados usando Fabric CA"
    ${CONTAINER_CLI_COMPOSE} -f compose/$COMPOSE_FILE_CA -f compose/$CONTAINER_CLI/${CONTAINER_CLI}-$COMPOSE_FILE_CA up -d 2>&1

    . organizations/fabric-ca/registerEnroll.sh

    #Se o arquivo não existir, o bloco dentro do if será executado.
    while :; do
      if [ ! -f "organizations/fabric-ca/org1/tls-cert.pem" ]; then
        infoln "-------------------------------------------------------"
        infoln "organizations/fabric-ca/org1/tls-cert.pem não existe"
        infoln "-------------------------------------------------------"
        sleep 1
      else
        break
    ...
    fatalln "Criando Identidades Org1  \   "
    infoln " --- chamando o createOrg1 --- "

    createOrg1

    fatalln "Criando Identidades Org2  \   "
    infoln " --- chamando o createOrg2 --- "

    createOrg2

    fatalln "Criando Identidades da Ordem \  "
    infoln " --- chamando o createOrderer --- "

    createOrderer

  fi

  # infoln "________________________________________________________________________________ "
  # fatalln "Gerando arquivos CCP para Org1 e Org2 executando o ./organizations/ccp-generate.sh "
  # infoln "________________________________________________________________________________ "
...
# Subir os nós de peer e orderer usando o Docker Compose.
function networkUp() {
  infoln "Executando networkUp"
  checkPrereqs

  # Gerar artefatos se eles não existirem
  if [ ! -d "organizations/peerOrganizations" ]; then
    infoln "Gerando certificados e artefatos de organização"
    successln " ---- Chamando createOrgs ----- "
    createOrgs
  fi
  ...
  if [ "${DATABASE}" == "couchdb" ]; then
    infoln "database === couchdb"
    infoln "Usando o CouchDB como banco de dados é usado para armazenar o world state"
    infoln " compose files = ${COMPOSE_FILES} -f compose/${COMPOSE_FILE_COUCH} -f compose/${CONTAINER_CLI}/${CONTAINER_CLI}-${COMPOSE_FILE_COUCH}"
    COMPOSE_FILES="${COMPOSE_FILES} -f compose/${COMPOSE_FILE_COUCH} -f compose/${CONTAINER_CLI}/${CONTAINER_CLI}-${COMPOSE_FILE_COUCH}"
  else
    infoln " database: ${DATABASE}"
    infoln "Usando o LevelDB como banco de dados"
  fi

  infoln "Iniciando a rede"
  infoln "Rede Base: ${COMPOSE_FILE_BASE}"
  if [ "${DATABASE}" == "couchdb" ]; then
    warnln "CouchDB: ${COMPOSE_FILE_COUCH}"
  fi

  DOCKER_SOCK="${DOCKER_SOCK}" infoln "Docker Socket: ${DOCKER_SOCK}"

  infoln "___________________________________________________-"
  infoln "compose files: ${COMPOSE_FILES}"

  infoln "Subindo a rede"
  DOCKER_SOCK="${DOCKER_SOCK}" ${CONTAINER_CLI_COMPOSE} ${COMPOSE_FILES} up -d 2>&1

  $CONTAINER_CLI ps -a
  if [ $? -ne 0 ]; then
    fatalln "Não foi possível iniciar a rede"
  fi
}
...
function createChannel() {
  infoln " ----------------- Executando createChannel in network.sh  ----------------"
  # Bring up the network if it is not already up.
  bringUpNetwork="false"
  # Atribui o primeiro argumento passado para a função à variável local
  local bft_true=$1
  infoln " Atribui o primeiro argumento passado para a função à variável local | CONTAINER_CLI: $CONTAINER_CLI"
  ...
  # Calcula o comprimento da lista de contêineres.
  len=$(echo ${#CONTAINERS[@]})
  echo "Calculou a lista dos containers e obteve: $len"
...
if [ "$MODE" == "up" ]; then
  infoln "Iniciando nós com um tempo limite de CLI de '${MAX_RETRY}' tentativas e um atraso de CLI de '${CLI_DELAY}' segundos e usando o banco de dados '${DATABASE}' ${CRYPTO_MODE}"
  infoln "--- Chamando networkUp ---"
  networkUp
